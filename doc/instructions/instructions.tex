...
\item The chassis is to be constructed by each group.  These can be 3-D printed,
  ...
  visible on top of the chassis.  The bumper must be as accessible as the
...
\item [Accelerometer/motors]
  For the Wacky Hat,
  \begin{itemize}
  \item Demonstrate the output of accelerometer readings to a PC using USB CDC (as provided in the test apps).
  \item Map the accelerometer readings to two duty cycles for variable speed and steering control (forward and backward), and output the result to a PC using USB CDC.
  \end{itemize}

  For the Wacky Racer,
  \begin{itemize}
    \item Demonstrate the operation of the locomotion motors using PWM signals (as provided in the test apps).
    \item Provide variable speed and steering control for the locomotion motors (forward and backward) from a PC using USB CDC. 
    \end{itemize}
...
\textbf{If you cannot show the functionality of a previous milestone
  during any assessment, you will fail that assessment and lose any
...
\begin{flushleft}
  \begin{tabular}{l|l}
    ...
    Speed control of motors        & Determine speed from accelerometer \\
    Steering control               & Determine steering from accelerometer\\
...
\item [Week-7]\mbox{}\\[-0.4cm]
  ...
  \begin{itemize}
  \item Demonstrate blinky program if ready.
  \end{itemize}
